# baoyu-xhs-images：Skill 说明与安装

baoyu-xhs-images 是 [anbeime/skill](https://github.com/anbeime/skill) 本地技能库收录的技能，归类在 **内容创作与发布**。
...
```text
请使用 baoyu-xhs-images 这个 Skill，先说明你会读取哪些说明文件，再帮我完成这个任务：<写清楚你的具体目标>
```
...
find ~/src/anbeime-skill -iname '*baoyu*xhs*images*' -print
...
baoyu-xhs-images 的价值在于把“生成小红书图文系列（1-10张）”这类任务沉淀成可复用流程。安装时以实际 `SKILL.md` 为准；如果仓库中没有入口文件，就先定位来源，不要创建不可触发的空 Skill。
