# ai-drawio：Skill 说明与安装

ai-drawio 是 [anbeime/skill](https://github.com/anbeime/skill) 本地技能库收录的技能，归类在 **设计与可视化**。
...
| API 需求 | ❌ 否 |
| API 类型 | 纯本地处理（mxGraph） |

## 适合做什么

适合视觉设计、界面生成、素材创作和需要稳定风格约束的创意任务。 在原始技能管理表中，它的整合状态是：图表绘制专用。
...
```text
请使用 ai-drawio 这个 Skill，先说明你会读取哪些说明文件，再帮我完成这个任务：<写清楚你的具体目标>
```

它还可以和这些技能协同：[[obsidian-skills-integrated]]。
...
find ~/src/anbeime-skill -iname '*ai*drawio*' -print
...
ai-drawio 的价值在于把“流程图和图表生成技能”这类任务沉淀成可复用流程。安装时以实际 `SKILL.md` 为准；如果仓库中没有入口文件，就先定位来源，不要创建不可触发的空 Skill。
