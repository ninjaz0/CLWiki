# peers-advisory-group：Skill 说明与安装

peers-advisory-group 是 [anbeime/skill](https://github.com/anbeime/skill) 本地技能库收录的技能，归类在 **智能体协作**。
...
适合团队协作、知识管理、会议整理、项目推进和个人效率工作流。 在原始技能管理表中，它的整合状态是：专业咨询场景。
...
```text
请使用 peers-advisory-group 这个 Skill，先说明你会读取哪些说明文件，再帮我完成这个任务：<写清楚你的具体目标>
```
...
find ~/src/anbeime-skill -iname '*peers*advisory*group*' -print
...
peers-advisory-group 的价值在于把“同行顾问团技能”这类任务沉淀成可复用流程。安装时以实际 `SKILL.md` 为准；如果仓库中没有入口文件，就先定位来源，不要创建不可触发的空 Skill。
