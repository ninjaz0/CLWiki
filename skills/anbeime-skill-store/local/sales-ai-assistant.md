# sales-ai-assistant：Skill 说明与安装

sales-ai-assistant 是 [anbeime/skill](https://github.com/anbeime/skill) 本地技能库收录的技能，归类在 **产品与项目管理**。
...
适合把该领域的经验、步骤和注意事项固化成代理可复用的执行流程。 在原始技能管理表中，它的整合状态是：销售场景专用。
...
```text
请使用 sales-ai-assistant 这个 Skill，先说明你会读取哪些说明文件，再帮我完成这个任务：<写清楚你的具体目标>
```

它还可以和这些技能协同：[[product-manager-toolkit]]。
...
find ~/src/anbeime-skill -iname '*sales*ai*assistant*' -print
...
sales-ai-assistant 的价值在于把“销售AI助手”这类任务沉淀成可复用流程。安装时以实际 `SKILL.md` 为准；如果仓库中没有入口文件，就先定位来源，不要创建不可触发的空 Skill。
